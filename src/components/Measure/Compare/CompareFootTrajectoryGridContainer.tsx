  ...
  const footContainer = (data: CompareFootTrajectoryGridProps, isRight: boolean) => (
    <div className="flex flex-col gap-2 w-full md:w-[50%]">
      <div className="flex gap-4 items-center bg-sub100 border-b-2 border-sub200 py-1 px-4">
        ...
        <span className={`text-base ${isRight ? 'text-sub600' : 'text-black'}`}>{data.measure_date.slice(0, 11)}</span>
      </div>
  ...
  const SummaryContainer = (dynamicComment: string, kneeComment: string, isRight: boolean) => (
    <div className="flex flex-col gap-2 border-l-0 md:border-l-2 border-sub200 w-full md:w-[50%]">
      ...
      <div className="flex flex-col justify-center h-full py-2">
        <div className={`flex items-center justify-start text-base ${isRight ? 'text-sub600' : 'text-black'} px-4 whitespace-pre-line`}>{dynamicComment}</div> 
        <div className={`flex items-center justify-start text-base ${isRight ? 'text-sub600' : 'text-black'} px-4 whitespace-pre-line`}>{kneeComment}</div> 
      </div>
    </div>
  );

...
export interface ROMItemCardProps {
  ...
  setRomMeasureType : (ronSn: number, measureType: number) => void
}
const FIXED_SLOTS = 5;

export default function CompareROMItemCard ({
  data,
  setRomMeasureType,
} : ROMItemCardProps) {
  ...
  const chartData = useMemo(() => {
    const sorted = Object.entries(data.history_by_measure_type)
  ...
  const displayMaxAngle = getRomDisplayMaxAngle(data?.value_1_min);
  ...
  return (
    <div
      ...
      onClick={() => {
        if (setRomMeasureType) {
          setRomMeasureType(data.sn, data.measure_type)
        };
      }}
    >
      <div className="flex flex-col">
        <div className="flex items-center justify-between rounded-t-xl bg-mainBlue-100  dark:bg-mainBlue-900 px-4 py-2 w-full">
          ...
          <div className={` flex gap-2 text-sm font-semibold border-2 rounded-full px-2 py-1 items-center ${stateTextColor[data.score]} ${stateBorderColor[data.score]}`}>
            <div className={`w-4 h-4 rounded-full ${stateBGColor[data.score]}`}/>
            {stateString[data.score]}
          </div>
        </div>
        ...
        <div className="flex flex-col w-full min-h-32 gap-2 p-2">
          <div className="flex flex-col gap-1 px-2">
            ...
            <p className="text-base line-clamp-2 min-h-12">
              {data.howto}
            </p>
          </div>

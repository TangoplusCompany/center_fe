import { formatText } from "@/components/Util/FormatSummary";
...
const CompareSummaryUnit = ({
  ...
  }[summaryUnit1.risk_level] ?? "bg-sub300" : undefined;
  ...
  const score0 = getRiskScore(summaryUnit0.risk_level);
  const score1 = summaryUnit1 ? getRiskScore(summaryUnit1.risk_level) : undefined;
  ...
  const summaryMent = (summaryUnit: CompareSummaryUnitProps, isNext: boolean) => {
    ...
    return (
      <div className="flex-1">
        ...
        <div className="text-sm text-sub600 px-4 py-2 whitespace-pre-line">{formatText(summaryUnit.ment)}</div>
      </div>
    );
  };
